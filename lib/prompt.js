...
export function buildUserPrompt(birthDate, matrixResults) {
  ...
  lines.push('');
  lines.push(`Lütfen bu sayıları bir bütün olarak analiz eden, 2. tekil şahısa doğrudan hitap eden, akıcı paragraflarla yazılmış, en az 3500 kelimelik uzun bir karmik özet metni yaz. Kişiye doğrudan "sizde", "sizin" diye hitap et. Sayıları birbirine örerek kombine yorumlar yap. Soy karması, aile kökleri, anne-baba soylarından gelen enerjiler, somut ritüel önerileri (sokak hayvanı beslemek, ağaç dikmek, dua, niyet, su çalışmaları, sadaka vb.) metnin içinde doğal bir şekilde yer alsın. Sağlık temalarına da doğal cümlelerle değin (madde liste yapma, asla tıbbi tavsiye verme). Yaş döngülerine atıfta bulun. Başlık ve madde listesi KULLANMA, düz akıcı prose yaz. Hiçbir kişi adı veya marka adı kullanma.`);
